...
#define ENC_SR  ((0x7<<5) | 0x1F)   // System Reset command does not use an address.
                                //   It requires 0x1F, however.

// Maximum SPI frequency specified in data sheet
//#define ENC_MAX_SPI_FREQ    (20000000ul)    // Hz
#define ENC_MAX_SPI_FREQ    (8000000ul)    // Hz
...
#define mENCPrintVar(foo) \
    length = sprintf(myString, #foo": 0x%02x\n", foo); \
    usbPrint(myString, length); 

#define mENCPrintERegister(myreg) \
    length = sprintf(myString, #myreg": 0x%02x\n", ReadETHReg((BYTE)myreg).Val); \
    usbPrint(myString, length);

#define mENCPrintMRegister(myreg) \
    length = sprintf(myString, #myreg": 0x%02x\n", ReadMACReg((BYTE)myreg).Val); \
    usbPrint(myString, length);

#define mENCPrintConst(myconst) \
    length = sprintf(myString, #myconst": 0x%04lx\n", myconst); \
    usbPrint(myString, length);
...
void MACInit(void)
{
...
#elif defined(ENC__PIC24FJ__)

    //ENC_SPICON1L = 0x8520;
    ENC_SPICON1L = 0x0000;
    ENC_SPICON1bits.MODE16 = 0;
    ENC_SPICON1bits.SMP = 1;
    ...
    ENC_SPISTATL = 0x0000;
    ENC_SPISTATH = 0x0000;
    ENC_SPIBRGL = (GetPeripheralClock()-1ul)/2ul/ENC_MAX_SPI_FREQ;
...
bool MACGetHeader(MAC_ADDR *remote, uint8_t* type)
{
    ...
    if(header.NextPacketPointer > RXSTOP || ((BYTE_VAL*)(&header.NextPacketPointer))->bits.b0 ||
       ...
       header.StatusVector.bits.uint8_tCount > 1518u ||
